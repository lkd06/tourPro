...
public class CommentDAO_before {
	
	private Connection conn;

	public CommentDAO_before(Connection conn) {
		this.conn=conn;
	}
	...
	public List<CommentDTO> bestList(int start, int end){
		...
		try {
			...
			sql = "select * from (";
			sql += "select rownum rnum, data.* from (";
			sql+= "select imagefilename,product,id,subject,count,hitcount,created from commentboard order by count desc) data) ";
			sql += "where rnum>=? and rnum<=?";
